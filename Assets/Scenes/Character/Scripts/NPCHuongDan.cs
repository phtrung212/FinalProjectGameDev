...
public class NPCHuongDan : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            Debug.Log("cccc");
            if (collision.GetComponent<HuongDan>().isAttacking == false)
            {
                if (collision.GetComponent<HuongDan>().Experence.getLevel() >= level)
                {
            ...
            {
                collision.GetComponent<HuongDan>().addNote("*** Đại hiệp đang trong trạng thái chiến đấu không thể vượt cảnh kế tiếp ***");
            }
        }
        //}
